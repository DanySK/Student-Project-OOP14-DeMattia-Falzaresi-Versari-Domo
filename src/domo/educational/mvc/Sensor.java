 ...
 * 
 * This class represent a Standar Sensor with a typology enum
 *
 */
...
public class Sensor {
	...
	/**
	 * Constructor of this element has the difference to accept an enum "sType" as type of sensor. 
	 * @param sName name to enter in the sensor
	 * @param sId id of the sensor
	 * @param sType enum type of the sensor
	 */
	public Sensor(final String sName, final int sId, final SensorTypology sType) {
		this.id = sId;
		this.name = sName;
		this.alert = false;
		this.alertCount = 0;
